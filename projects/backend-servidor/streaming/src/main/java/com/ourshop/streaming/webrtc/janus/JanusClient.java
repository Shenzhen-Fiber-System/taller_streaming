...
public class JanusClient {
    ...
    public JanusClient(
        ...
        this.webClient = WebClient.builder().baseUrl(janusUrl).build();
    ...
    public Mono<Long> createSession() {
        ...
        return post("/janus", payload)
                ...
                .flatMap(id -> id > 0 ? Mono.just(id) : Mono.error(new IllegalStateException("Invalid session id from Janus")));
    }
    ...
    public Mono<Long> attachPlugin(long sessionId, String pluginName) {
        ...
        return post("/janus/" + sessionId, payload)
                .map(json -> json.path("data").path("id").asLong())
                .flatMap(id -> id > 0 ? Mono.just(id) : Mono.error(new IllegalStateException("Invalid handle id from Janus")));
    }
    ...
    public Mono<Void> sendTrickleCandidate(long sessionId, long handleId, String candidate, String sdpMid, Integer sdpMLineIndex) {
